...
circle_deals_screen_target = r"@OptIn\(ExperimentalMaterial3Api::class\)\s*@Composable\s*fun CircleDealsScreen\(.*?}\s*}\s*}"

circle_deals_screen_replacement = """@OptIn(ExperimentalMaterial3Api::class)
@Composable
fun CircleDealsScreen(onBack: () -> Unit = {}, onProductClick: (Product) -> Unit = {}) {
    ...
    var circleDealsList by remember { mutableStateOf(mockProducts) }
    ...
    LaunchedEffect(Unit) {
        while (true) {
            ...
            if (circleDealsList.isNotEmpty()) {
                ...
                if (product.stock > 0) {
                    val updatedProduct = product.copy(stock = product.stock - 1, soldCount = product.soldCount + 1)
            ...
            items(chunkedProducts) { rowProducts ->
                Row(
                    ...
                    for (product in rowProducts) {
                        CircleDealCard(product, modifier = Modifier.weight(1f).clickable { onProductClick(product) })
                    }
...
content = re.sub(circle_deals_screen_target, circle_deals_screen_replacement, content, flags=re.DOTALL)
